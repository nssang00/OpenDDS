...
namespace EAtoIDL.Extractor
{
    ...
    public class EAModelExtractor
    {
        ...
        {
            // 속성(필드) 추출
            // attr.ClassifierID 가 0이면 원시 타입(double, long 등).
            // 0보다 크면 모델 내 커스텀 타입 → ResolveAllReferences() 에서 해석합니다.
            foreach (EA.Attribute attr in eaElem.Attributes)
            {
                elem.Fields.Add(new ModelField
                {
                    ...
                    ClassifierID = attr.ClassifierID,   // 0이면 원시 타입
        ...
        private void ExtractDataType(EA.Element eaElem, ModelElement elem)
        {
            ...
            if (eaElem.Attributes.Count > 0)
            {
                ...
                // ClassifierID > 0 이면 모델 요소 타입 → ResolveAllReferences() 에서 해석
                if (first.ClassifierID > 0)
                {
                    var classifier = _repo.GetElementByID(first.ClassifierID);
                    if (classifier != null)
        ...
        private void ResolveAllReferences()
        {
            foreach (var elem in _byGuid.Values)
            {
                ...
                // 필드 타입 해석
                // ClassifierID(int) 로 EA 요소를 조회한 뒤 _byGuid 에서 ModelElement 를 찾습니다.
                foreach (var field in elem.Fields)
                {
                    if (field.ClassifierID > 0)
                    {
                        var eaType = _repo.GetElementByID(field.ClassifierID);
                        if (eaType != null)
                        {
                            ModelElement typeElem;
                            if (_byGuid.TryGetValue(eaType.ElementGUID, out typeElem))
