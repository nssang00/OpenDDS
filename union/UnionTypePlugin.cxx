    ...
    const struct DDS_TypeAllocationParams_t * alloc_params) 
{
    ...
    sample = new (std::nothrow) UnionType();
    if (sample == NULL) {
        return NULL;
    }

    if (!UnionType_initialize_w_params(sample,alloc_params)) {
        ...
        UnionType_finalize_w_params(sample, &deallocParams);
        ...
        is freed in the call "UnionType_finalize_ex" */
...
UnionTypePluginSupport_create_data_ex(RTIBool allocate_pointers) 
{
    ...
    if (!UnionType_initialize_ex(sample,allocate_pointers, RTI_TRUE)) {
        /* Coverity reports a possible uninit_use_in_call that will happen if the
...
{
    return UnionTypePluginSupport_create_data_ex(RTI_TRUE);
}
...
UnionTypePluginSupport_destroy_data_w_params(
    ...
    UnionType_finalize_w_params(sample,dealloc_params);
...
UnionTypePluginSupport_destroy_data_ex(
    ...
    UnionType_finalize_ex(sample,deallocate_pointers);
...
UnionTypePluginSupport_destroy_data(
    ...
    UnionTypePluginSupport_destroy_data_ex(sample,RTI_TRUE);
...
UnionTypePluginSupport_copy_data(
    ...
    const UnionType *src)
{
    return UnionType_copy(dst,(const UnionType*) src);
}
    ...
    unsigned int indent_level)
{
    ...
    switch(sample->_d) {
        ...
        case 11:
        {
            ...
            RTICdrType_printLong(
                &sample->_u.long_data, "_u.long_data", indent_level + 1);    
        ...
        case 22:
        {
            ...
            RTICdrType_printFloat(
                &sample->_u.float_data, "_u.float_data", indent_level + 1);    
        ...
        case 33:
        {
            if (sample->_u.string_data==NULL) {
                RTICdrType_printString(
                    NULL,"_u.string_data", indent_level + 1);
            } else {
                RTICdrType_printString(
                    sample->_u.string_data,"_u.string_data", indent_level + 1);    
            }
...
UnionTypePlugin_return_sample(
    ...
    void *handle)
{
    UnionType_finalize_optional_members(sample, RTI_TRUE);

    PRESTypePluginDefaultEndpointData_returnSample(
        endpoint_data, sample, handle);
}

void UnionTypePlugin_finalize_optional_members(
    PRESTypePluginEndpointData endpoint_data,
    UnionType* sample,
    RTIBool deletePointers)
{
    RTIOsapiUtility_unusedParameter(endpoint_data);
    UnionType_finalize_optional_members(
    ...
    DDS_DataRepresentationId_t representation)
{
    ...
    UnionType_get_typecode();
    pd.programs = UnionTypePlugin_get_programs();
    if (pd.programs == NULL) {
        return RTI_FALSE;
    }
...
UnionTypePlugin_serialize_to_cdr_buffer(
    ...
    const UnionType *sample)
{
    return UnionTypePlugin_serialize_to_cdr_buffer_ex(
        buffer,
        length,
        sample,
    ...
    const struct DDS_PrintFormatProperty *property)
{
    ...
    if (!UnionTypePlugin_serialize_to_cdr_buffer(
        NULL, 
        &length, 
        sample)) {
...
{ 
    ...
    UnionTypePlugin_finalize_optional_members;
    ...
    */
    ...
    #else
    plugin->typeCode =  (struct RTICdrTypeCode *)UnionType_get_typecode();
    #endif
    ...
    plugin->endpointTypeName = UnionTypeTYPENAME;
