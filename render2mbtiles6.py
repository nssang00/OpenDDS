...
def init_mbtiles(path, name, scheme):
    ...
    if new:
        ...
        meta={"name":name,"type":"baselayer","version":"1",
...
def main():
    ...
    with init_mbtiles(args.mbtiles, os.path.basename(args.mbtiles), args.scheme) as conn:
        cur=conn.cursor(); insert_sql="INSERT OR REPLACE INTO tiles(zoom_level,tile_column,tile_row,tile_data) VALUES (?,?,?,?)"
        for z in zooms:
            ...
            n=2**z; xmin=max(0,min(xmin,n-1)); xmax=max(0,min(xmax,n-1))
