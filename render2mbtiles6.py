...
def lonlat_to_merc(lon, lat):
    x=R*math.radians(lon); y=R*math.log(math.tan(math.pi/4 + math.radians(clamp_lat(lat))/2)); return x,y
def lonlat_to_tile(lon, lat, z):
    n=2**z; r=math.radians(clamp_lat(lat))
...
def parse_zoom(spec):
    ...
    for p in str(spec).split(","):
        ...
        if "-" in p: a,b=map(int,p.split("-",1)); zs.update(range(a,b+1))
        else: zs.add(int(p))
    return sorted(zs)
def parse_bbox(s):
    try:
...
def setup_mbtiles(path,name,scheme,minzoom,maxzoom,bounds):
    ...
    if new:
        ...
        for k,v in {"name":name,"type":"baselayer","version":"1","description":"Rendered tiles",
                    "format":"png","minzoom":minzoom,"maxzoom":maxzoom,"bounds":bounds,"scheme":scheme}.items():
...
def commit_progress(conn,pending,threshold,z,written,total,final=False):
    if pending>=threshold or (final and pending>0):
        conn.commit(); pct=(written/total) if total else 1.0
