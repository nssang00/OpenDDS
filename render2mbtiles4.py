...
def worker_init(xml_path, tilesize, bg, xmin, xmax, ymin, ymax):
...
# --------- 메인 ---------
def main():
    ap = argparse.ArgumentParser(description="Render zoom range (zmin~zmax) into MBTiles with meta-tiles (8x8, png256, multiprocessing).")
    ...
    for z in range(args.zmin, args.zmax + 1):
        ...
        with mp.Pool(
            ...
            initargs=(args.xml, args.tilesize, args.bg, args.xmin, args.xmax, args.ymin, args.ymax),
            maxtasksperchild=50,
        ) as pool:
            for rows in pool.imap_unordered(render_meta_tile, tasks, chunksize=1):
                ...
                if pending >= args.commit_batch:
                    conn.commit()
                    print(f"[z{z}] +{pending} tiles committed ({written}/{total_tiles}, {written/total_tiles:.1%})")
                    pending = 0
            if pending:
                conn.commit()
                print(f"[z{z}] +{pending} tiles committed (final)")
