...
def main():
    ...
    # DB 오픈: minzoom/maxzoom 메타데이터를 실제 요청 범위로 기록
    ...
    for z in range(args.zmin, args.zmax + 1):
        ...
        # 진단 로그 (중요!)
        print(f"\n=== z={z} ===")
        print(f"range x:[{xmin_z},{xmax_z}]  y:[{ymin_z},{ymax_z}]  tiles≈{total_tiles}")
        print(f"tasks meta-start x:{mx0}->{xmax_z} step {META}, y:{my0}->{ymax_z} step {META}  count={len(tasks)}")
