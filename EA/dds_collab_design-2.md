...
| t_object | YES | All Class/Interface/Enumeration/DataType within scope |
...
#### Step 2. Read extractable t_object within target packages
```sql
...
```
Note: All Class/Interface/Enumeration/DataType are extracted regardless of stereotype.
DDS-specific stereotypes (struct, idlStruct, enumeration, union, typedef, alias)
are used as hints for kind resolution, not as filters.
...
Object_Type = 'Class' | 'Interface' | 'Enumeration' | 'DataType'
  -> types
  (kind resolution: see types.kind rules below)

types.kind resolution:
  Stereotype = 'struct' | 'idlStruct'    -> kind = 'struct'
  Stereotype = 'enumeration'             -> kind = 'enum'
  Stereotype = 'union'                   -> kind = 'union'
  Stereotype = 'typedef' | 'alias'       -> kind = 'typedef'
  Object_Type = 'Enumeration'            -> kind = 'enum'
  Object_Type = 'DataType'               -> kind = 'typedef'
  Object_Type = 'Interface'              -> kind = 'struct'
  Object_Type = 'Class' (no stereotype)  -> kind = 'struct' (default)
...
### Name Sanitization
```cpp
// Applied to all module names, struct/enum/union/typedef names, and type references
static std::string toIdlName(const std::string& name) {
    // Replace space, hyphen, period, colon with underscore
    // e.g. "Hello World" -> "Hello_World"
    //      "Net Chat"    -> "Net_Chat"
    //      "DDS-Topic"   -> "DDS_Topic"
}
```
toIdlName() is applied to: module names, struct/enum/union/typedef names, ref_type_name, base_type_name.
