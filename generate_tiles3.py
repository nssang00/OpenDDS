...
def main():
    ...
    print(f"타일 계산 시간: {format_time(calc_time)}")
    ...
    print(f"데이터베이스 초기화 시간: {format_time(db_init_time)}")
    ...
    with Pool(processes=args.processes) as pool:
        for tiles_data in pool.imap_unordered(render_metatile, all_metatiles):
            if tiles_data:
                ...
                tiles_per_sec = rendered / elapsed if elapsed > 0 else 0
                remaining = (total_tiles - rendered) / tiles_per_sec if tiles_per_sec > 0 else 0
                
                print(f"Progress: {rendered}/{total_tiles} tiles ({rendered*100//total_tiles}%) | "
                      f"{tiles_per_sec:.1f} tiles/s | ETA: {format_time(remaining)}", end='\r')
    ...
    print(f"\n\n{'='*60}")
    print(f"Complete! {rendered} tiles rendered to {args.output}")
    print(f"{'='*60}")
    print(f"타일 계산 시간:    {format_time(calc_time)}")
    print(f"DB 초기화 시간:    {format_time(db_init_time)}")
    print(f"렌더링 시간:       {format_time(render_time)}")
    print(f"{'='*60}")
    print(f"총 소요 시간:      {format_time(total_time)}")
    print(f"평균 렌더링 속도:  {rendered/render_time:.2f} tiles/s")
    print(f"{'='*60}")
