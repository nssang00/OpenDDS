...
import {
  ...
  generateLineStringRenderInstructionsFromFeatures,//kmg
...
//kmg
import MixedGeometryBatch from './MixedGeometryBatch.js';
...
class VectorStyleRenderer {
  ...
  constructor(styleOrShaders, variables, helper, enableHitDetection, filter) {
    ...
    //kmg
    this.contextFilter_ = null;
    if (filter) {
      const { featureFilters, contextFilters } = splitFilters(filter);
      this.featureFilter_ = this.computeFeatureFilter(featureFilters);
      this.contextFilter_ = this.computeContextFilter(contextFilters);;
    }
...
/*
   ...
   * @private
   */
  generateRenderInstructions_(geometryBatch, transform, featuresBatch) {
  ...
  renderInternal_(
    ...
    //kmg
    if (this.contextFilter_) {
      if(!this.contextFilter_(frameState.viewState.resolution)) {
        return;
      }
    }
