...
function generateLineStringRenderInstructionsFromFeatures(
  ...
  for (let i = 0; i < features.length; i++) {
    ...
    geometryRenderEntries[i] = { feature, pixelCoordinates, ends, flatCoordinates, stride };
  }

  // here we anticipate the amount of render instructions for lines:
  // 7 instructions per vertex for position (x, y, m, angle0, angle1, currentLength, currentAngleTangentSum)
  // + 1 instruction per line per custom attributes
  // + 1 instruction per line (for vertices count)
  const totalInstructionsCount =
    7 * verticesCount +
  ...
  for (const entry of geometryRenderEntries) {
    ...
    for (const end of entry.ends) {
      ...
      // Pre-calculate world coordinates for angle calculations
      const worldCoordinates = [];
      for (let i = offset; i < end; i += stride) {
        worldCoordinates.push([
          entry.flatCoordinates[i],
          entry.flatCoordinates[i + 1],
          stride === 3 ? entry.flatCoordinates[i + 2] : 0
        ]);
      }
      
      // Check if this is a loop (first and last points are the same)
      const isLoop = worldCoordinates.length > 2 && 
      ...
      for (let i = 0; i < worldCoordinates.length; i++) {
        ...
        // Calculate angle0 (angle at current point from previous point)
        if (i > 0) {
          const prevPoint = worldCoordinates[i - 1];
          const nextPoint = i < worldCoordinates.length - 1 ? worldCoordinates[i + 1] : 
                           (isLoop ? worldCoordinates[1] : null);
          if (nextPoint) {
            angle0 = angleBetween(worldPoint, nextPoint, prevPoint);
            if (Math.cos(angle0) <= 0.985) { // LINESTRING_ANGLE_COSINE_CUTOFF
              newAngleTangentSum += Math.tan((angle0 - Math.PI) / 2);
            }
          }
        } else if (isLoop && worldCoordinates.length > 2) {
          const prevPoint = worldCoordinates[worldCoordinates.length - 2];
          const nextPoint = worldCoordinates[1];
          angle0 = angleBetween(worldPoint, nextPoint, prevPoint);
        ...
        // Calculate angle1 (angle at next point from current point)
        if (i < worldCoordinates.length - 1) {
          const nextPoint = worldCoordinates[i + 1];
          const afterPoint = i < worldCoordinates.length - 2 ? worldCoordinates[i + 2] : 
                            (isLoop ? worldCoordinates[1] : null);
          if (afterPoint) {
            angle1 = angleBetween(nextPoint, worldPoint, afterPoint);
          ...
          const nextPoint = worldCoordinates[1];
          const afterPoint = worldCoordinates[2];
          angle1 = angleBetween(nextPoint, worldPoint, afterPoint);
          if (Math.cos(angle1) <= 0.985) {
            newAngleTangentSum += Math.tan((Math.PI - angle1) / 2);
          }
        ...
        // Store in render instructions
        renderInstructions[renderIndex++] = entry.pixelCoordinates[pixelIndex]; // x
        renderInstructions[renderIndex++] = entry.pixelCoordinates[pixelIndex + 1]; // y
        renderInstructions[renderIndex++] = worldPoint[2]; // m
        ...
        // Update currentLength for next iteration
        if (i < worldCoordinates.length - 1) {
          const nextPoint = worldCoordinates[i + 1];
          currentLength += Math.sqrt(
            (nextPoint[0] - worldPoint[0]) * (nextPoint[0] - worldPoint[0]) +
            (nextPoint[1] - worldPoint[1]) * (nextPoint[1] - worldPoint[1])
          );
        }
...
function generateLineStringBuffers_(renderInstructions, customAttributesSize, transform) {
  const customAttrsCount = customAttributesSize;
  const instructionsPerVertex = 7; // x, y, m, angle0, angle1, currentLength, currentAngleTangentSum
  ...
  while (currentInstructionsIndex < renderInstructions.length) {
    ...
    for (let i = 0; i < verticesCount - 1; i++) {
      ...
      const m0 = renderInstructions[segmentStartIndex + 2];
      const m1 = renderInstructions[segmentEndIndex + 2];
      const angle0 = renderInstructions[segmentStartIndex + 3];
      const angle1 = renderInstructions[segmentEndIndex + 3];
      const currentLength = renderInstructions[segmentStartIndex + 5];
      const currentAngleTangentSum = renderInstructions[segmentStartIndex + 6];
...
function generateLineStringRenderInstructionsFromFeatures(
  ...
  for (let i = 0; i < features.length; i++) {
    ...
    geometryRenderEntries[i] = { feature, pixelCoordinates, flatCoordinates, ends, stride };
  }

  // 7: x, y, m, angle0, angle1, currentLength, currentAngleTangentSum
  const totalInstructionsCount =
    7 * verticesCount +
    (1 + getCustomAttributesSize(customAttributes)) * geometriesCount;
  ...
  for (const entry of geometryRenderEntries) {
    ...
    for (const end of entry.ends) {
      ...
      const isLoop = verticesInLine > 2 &&
        entry.flatCoordinates[firstIdx] === entry.flatCoordinates[lastIdx] &&
        entry.flatCoordinates[firstIdx + 1] === entry.flatCoordinates[lastIdx + 1];
      ...
      for (let i = 0; i < verticesInLine; i++) {
        const getCoord = (vertexIdx) => {
          ...
          return [
            entry.flatCoordinates[idx],
            entry.flatCoordinates[idx + 1]
          ];
        };
        const getM = (vertexIdx) => {
          const idx = offset + vertexIdx * stride;
          return stride > 2 ? entry.flatCoordinates[idx + 2] : 0;
        };
        ...
        if (i > 0) {
          ...
          const next = (i < verticesInLine - 1)
            ? getCoord(i + 1)
            : (isLoop ? getCoord(1) : null);
          if (next) {
            angle0 = angleBetween(getCoord(i), next, prev);
            if (Math.cos(angle0) <= 0.985) {
              newAngleTangentSum += Math.tan((angle0 - Math.PI) / 2);
            }
          }
        ...
        if (i < verticesInLine - 1) {
          ...
          const after = (i < verticesInLine - 2)
            ? getCoord(i + 2)
            : (isLoop ? getCoord(1) : null);
          if (after) {
            angle1 = angleBetween(next, getCoord(i), after);
            if (Math.cos(angle1) <= 0.985) {
              newAngleTangentSum += Math.tan((Math.PI - angle1) / 2);
            }
          }
        } else if (isLoop && verticesInLine > 2 && i === verticesInLine - 1) {
          const next = getCoord(1);
        ...
        renderInstructions[renderIndex++] = entry.pixelCoordinates[idx];      // x
        renderInstructions[renderIndex++] = entry.pixelCoordinates[idx + 1];  // y
        ...
        if (i < verticesInLine - 1) {
          ...
          currentLength += Math.sqrt(
            (entry.flatCoordinates[nextIdx] - entry.flatCoordinates[idx]) ** 2 +
            (entry.flatCoordinates[nextIdx + 1] - entry.flatCoordinates[idx + 1]) ** 2
          );
        }
        currentAngleTangentSum = newAngleTangentSum;
      }
