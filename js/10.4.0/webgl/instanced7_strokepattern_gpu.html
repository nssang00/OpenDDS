...
float angleBetween(vec2 p0, vec2 pA, vec2 pB) {
  float lenA = length(pA - p0);
  vec2 tangentA = (lenA > 0.0) ? (pA - p0) / lenA : vec2(0.0);
  vec2 orthoA = vec2(-tangentA.y, tangentA.x);
  
  float lenB = length(pB - p0);
  vec2 tangentB = (lenB > 0.0) ? (pB - p0) / lenB : vec2(0.0);
  
  float angle = (lenA == 0.0 || lenB == 0.0) ? 0.0 : 
                acos(clamp(dot(tangentB, tangentA), -1.0, 1.0));
  
  bool isClockwise = dot(tangentB, orthoA) > 0.0;
