...
/////
using ResultCallback = std::function<void(const Variant&)>;
...
void processArgs(const std::vector<Variant>& args) {
    ...
    if (!args.empty()) {
        ...
        if (lastArg.type() == typeid(ResultCallback)) {
            //resultCallback = std::any_cast<const std::function<void(const Variant&)>&>(lastArg);
            const auto& resultCallback = std::any_cast<const ResultCallback&>(lastArg);
            if (resultCallback) {
                Variant variant = 42;
