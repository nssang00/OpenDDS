...
function parseXML(xmlString) {
  ...
  for (let style of styleNodes) {
    ...
    switch (styleObj.type) {
      case "point":
        symbolizers = parsePointLayers(style);
        break;
      case "line":
        symbolizers = parseLineLayers(style);
        break;
      case "polygon":
        symbolizers = parsePolygonLayers(style);
        break;
      case "Label":
        symbolizers = parseLabelLayers(style);
        break;
      default:
        break;
    }
  ...
  function parsePolygonLayers(styleNode) {
    return {
        PolygonLayers: Array.from(styleNode.getElementsByTagName("PolygonLayer")).map(layer => {
            const layerObj = { type: layer.getAttribute("type") };
            //Array.from(layer.children).forEach(child => {
            for (let child of layer.children) {
            ...
            Object.assign(layerObj, parseLineLayers(layer));
