...
class VectorStyleRenderer {
  ...
  constructor(styleOrShaders, helper, enableHitDetection) {
    ...
    const hitDetectionAttributes = this.hitDetectionEnabled_
      ...
      : { hitColor: { callback: () => [0, 0, 0, 0], size: 4 } };
  ...
  async generateBuffers(geometryBatch, transform, features) {
    ...
    const filteredFeatures = features.filter((feature) => {
      return this.evaluators_.featureFiltersEvaluator({
    ...
    */
    ...
    console.log('renderInstructions', features, geometryBatch, result);
   ...
   * @private
   */
  generateRenderInstructions_(geometryBatch, transform, geometryBatch2) {//kmg
    ...
    const lineStringInstructions = this.hasStroke_
      ? generateLineStringRenderInstructions(
          ...
          this.customAttributes_,
          transform,
          geometryBatch2.lineStringFeatures
        )
      : null;
  ...
  renderInternal_(
    ...
    if (!this.evaluators_.contextfiltersEvaluator({
      ...
      console.log('filterEvaluator_', frameState.viewState.resolution);
      return ;
    }
