...
void* CustomAllocator::allocate(size_t size) {
    ...
    return memBlock ? static_cast<void*>(memBlock->payload) : NULL;
}
...
void CustomAllocator::free(void* object, size_t size) {
    ...
    if (memBlock->signature != HEADER_SIGNATURE || *footer != FOOTER_SIGNATURE) {
        (size == 1) ? delete[] reinterpret_cast<unsigned char*>(object) : delete[] reinterpret_cast<unsigned char*>(object);
        return;
    }
